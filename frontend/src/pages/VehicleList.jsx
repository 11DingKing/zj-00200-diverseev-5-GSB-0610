...
function VehicleList() {
  ...
  const loadComparisonCount = useCallback(async () => {
    try {
      const res = await comparisonAPI.getCount();
      setComparisonCount(res.data.count);
    } catch (error) {
      console.error("加载对比集数量失败:", error);
    }
  }, []);

  const loadComparisonIds = useCallback(async () => {
    try {
      const res = await comparisonAPI.getList();
      setComparisonIds(new Set(res.data.map((v) => v.id)));
    } catch (error) {
  ...
  useEffect(() => {
    ...
    loadComparisonCount();
    loadComparisonIds();
  }, [filters, loadComparisonCount, loadComparisonIds]);
  ...
  const handleAddToComparison = async (vehicle) => {
    try {
      await comparisonAPI.addVehicle(vehicle.id);
      message.success(`${vehicle.brand} ${vehicle.name} 已加入对比集`);
      loadComparisonCount();
      loadComparisonIds();
    } catch (error) {
      if (error.response) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          <Button
            type={comparisonIds.has(record.id) ? "default" : "link"}
            size="small"
            icon={<SwapOutlined />}
            disabled={comparisonIds.has(record.id)}
            onClick={() => handleAddToComparison(record)}
          >
            {comparisonIds.has(record.id) ? "已加入" : "对比"}
          </Button>
          <Button
            type="link"
            size="small"
    ...
    <div>
      <Card className="filter-section">
        <Row gutter={[16, 16]} align="middle">
          ...
          <Col>
            <Badge count={comparisonCount} size="small" offset={[-4, 4]}>
              <Button
                icon={<SwapOutlined />}
