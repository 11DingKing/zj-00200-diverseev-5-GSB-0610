...
function VehicleList() {
  ...
  const handleAddToComparison = async (vehicle) => {
    try {
      ...
      message.success(`${vehicle.brand} ${vehicle.name} 已加入对比`);
      loadComparisonSet();
    } catch (error) {
      message.error(error.response?.data?.error || "加入对比失败");
    }
  };

  const isInComparison = (vehicleId) => {
    return comparisonSet.vehicles.some((v) => v.id === vehicleId);
  };
      ...
      render: (_, record) => (
        <Space size="small">
          <Button
            ...
            disabled={isInComparison(record.id)}
          >
            {isInComparison(record.id) ? "已加入" : "加入对比"}
          </Button>
          <Button
            type="link"
            size="small"
    ...
    <div>
      <Card className="filter-section">
        <Row gutter={[16, 16]} align="middle">
          ...
          <Col>
            <Badge count={comparisonSet.count} size="small">
              <Button
                type="primary"
                ghost
                icon={<SwapOutlined />}
                onClick={() => navigate("/comparison")}
                disabled={comparisonSet.count === 0}
              >
                对比 ({comparisonSet.count}/{comparisonSet.maxSize})
              </Button>
            </Badge>
          </Col>
        </Row>
